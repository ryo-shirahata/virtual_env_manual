...
今回は vagrant というユーザーでゲストOS内にログインしているため
...
本来一つのサーバではApacheかNginxどちらか片方しか使用しませんが、

今回は練習としてApache、Nginx両方のパターンで試してみましょう。

Nginxの方がApacheよりもWebサーバとしてのシェア率が高く、
...
これはApacheが mod_php というモジュールを使用して
...
では変更した routes/web.php の内容を元に戻して再度 http://192.168.33.19 にアクセスして
...
## エラーの確認
Apacheの設定ファイルに記述ミスなどが合った場合に

CentOSでは /etc/httpd/logs/error_log というファイルにエラーの内容が書き込まれます。
